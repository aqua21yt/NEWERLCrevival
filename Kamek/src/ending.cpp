...
dScEnding_c *dScEnding_c::instance = 0;

dScEnding_c *dScEnding_c::build() {
	// return new dScEnding_c;
	void *buffer = AllocFromGameHeap1(sizeof(dScEnding_c));
	dScEnding_c *c = new(buffer) dScEnding_c;
...
bool Opening_DoLoading() {
	DVD_LoadFile(GetDVDClass(), "Object", "EndingPics", 0);
...
int dScEnding_c::onCreate() {
	...
	*CurrentDrawFunc = OpeningDrawFunc;
    ...
    if(GetGameLanguage(4) == 0) {openingFile = (OpeningBinary*)LoadFile(&fileHandle, "/NewerRes/EndingJP.bin");}
	if(GetGameLanguage(4) == 1) {openingFile = (OpeningBinary*)LoadFile(&fileHandle, "/NewerRes/EndingEN.bin");}
	if(GetGameLanguage(4) == 2) {openingFile = (OpeningBinary*)LoadFile(&fileHandle, "/NewerRes/EndingGE.bin");}
	if(GetGameLanguage(4) == 3) {openingFile = (OpeningBinary*)LoadFile(&fileHandle, "/NewerRes/EndingFR.bin");}
	if(GetGameLanguage(4) == 4) {openingFile = (OpeningBinary*)LoadFile(&fileHandle, "/NewerRes/EndingSP.bin");}
	if(GetGameLanguage(4) == 5) {openingFile = (OpeningBinary*)LoadFile(&fileHandle, "/NewerRes/EndingIT.bin");}
	if(GetGameLanguage(4) == 6) {openingFile = (OpeningBinary*)LoadFile(&fileHandle, "/NewerRes/EndingDU.bin");}

	return true;
}

int dScEnding_c::onDelete() {
	FreeFile(&fileHandle);
	DVD_FreeFile(GetDVDClass2(), "EndingPics");
	return layout.free();
}

extern void callOpeningState(dScEnding_c* handler, u32 state);
int dScEnding_c::onExecute() {
...
void dScEnding_c::state1_whatever() {
...
void dScEnding_c::state2_whatever() {
	if (!layout.isAnimOn(3)) {
		layout.disableAllAnimations();
		layout.enableNonLoopAnim(6);
		currentStateID++;
...
void dScEnding_c::state3_whatever() {
...
void dScEnding_c::state4_whatever() {
	...
	if(nowPressed & WPAD_TWO) {
		...
		if(this->currentPicture < this->openingFile->pagesCount-1) {
			PlaySoundWithFunctionB4(SoundRelatedClass, &handles[0], SE_SYS_DECIDE, 1);
			currentStateID++;
		}
		else {
			currentStateID += 4;
...
void dScEnding_c::state5_whatever() {
...
void dScEnding_c::state6_whatever() {
...
void dScEnding_c::state7_whatever() {
	if (!layout.isAnimOn(5)) {
		this->loadPrevPic();
		layout.disableAllAnimations();
...
void dScEnding_c::state8_whatever() {
...
void dScEnding_c::loadIdlePic() {
	...
	u8 *leftBuf = getResource("EndingPics", leftName);
	...
	u8 *rightBuf = getResource("EndingPics", rightName);
